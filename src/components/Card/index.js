...
function Card({ transaction, deleteTransaction }) {
  return (
    <div
      className={
        transaction.type === "Despesa" ? "div__Despesa" : "div__Entrada"
      }
    >
      <div className="transaction">
        <h3 className="transaction__Description">{transaction.description}</h3>
        <div className="transaction__DivValueDelete">
      ...
      <p className="transaction__Type">{transaction.type}</p>
    </div>
  );
}
